...
arma::colvec test_ols(const arma::mat& X, 
                      const arma::colvec& y, 
                      arma::colvec beta);
...
double update_d(const arma::mat& A,
                const arma::colvec& u, 
                const arma::colvec& v,
                const arma::colvec& u_svd, 
                const arma::colvec& v_svd,
                bool u_only,
                bool v_only); 
...
int update_u_3(const arma::mat& A,
               const arma::mat& X,
               arma::colvec& u, 
               const arma::colvec& v, 
               const arma::colvec& y, 
...
int update_v_3(const arma::mat& A,
               const arma::mat& X,
               const arma::colvec& u, 
               arma::colvec& v, 
               const arma::colvec& y, 
...
double compute_objective(const arma::mat& A, 
                         const arma::mat& X, 
                         const arma::colvec& y, 
                         const arma::colvec& beta, 
                         double d, 
              ...
              int only_v = 0) 
{
  ...
  if(A.is_symmetric()) {
    ...
    if(!only_u && !only_v) {
      iter = lr_(A, X, y, u, v, beta, u_distance, d, l, tol, max_iter, verbose, 
                 false, early_stopping);  
    } else {
      
      iter = lr_(A, X, y, u, v, beta, u_distance, d, l, tol, max_iter, verbose, 
                 false, early_stopping, only_u, only_v);
    }
...
// [[Rcpp::export]]
Rcpp::List lr_rand_int(const arma::mat& A, 
                       const arma::mat& X, 
                       const arma::colvec& y,
                       double d,
                       arma::colvec& u,
                       ...
                       int scaled = 1) 
{
  ...
  // output
  Rcpp::List ret_list;
  arma::colvec beta(k + 2, fill::zeros), residuals(n), u_distance(max_iter);
  int iter;
  Rcpp::StringVector method = "lr";
  
  iter = lr_(A, X, y, u, v, beta, u_distance, d, l, tol, max_iter, verbose, true);
...
// [[Rcpp::export]]
Rcpp::List lr_test(const arma::mat& A, 
                   const arma::mat& X, 
                   const arma::colvec& y,
                   double l, 
                   double tol,
                   ...
                   int scaled = 1) 
{
  ...
  // output
  Rcpp::List ret_list;
  arma::colvec u, v, beta(k + 2, fill::zeros), residuals(n), obj(max_iter), u_distance(max_iter), v_distance(max_iter);
  double d;
  int iter;
  Rcpp::StringVector method = "lr";
  
  
  iter = lr_(A, X, y, u, v, beta, obj, u_distance, v_distance, d, l, tol, max_iter, verbose, false, u0, v0);
  ...
  ret_list = Rcpp::List::create(_("u") = u,
                                ...
                                _("obj") = obj.head(iter),
                                _("u_distance") = u_distance.head(iter),
                                _("v_distance") = v_distance.head(iter));
...
// [[Rcpp::export]]
Rcpp::List lr_rand_init_test(const arma::mat& A, 
                             const arma::mat& X, 
                             const arma::colvec& y,
                             double d,
                             arma::colvec& u,
                             ...
                             int scaled = 1) 
{
  ...
  // output
  Rcpp::List ret_list;
  arma::colvec beta(k + 2, fill::zeros), residuals(n), obj(max_iter), u_distance(max_iter), v_distance(max_iter);
  int iter;
  Rcpp::StringVector method = "lr";
                 ...
                 Nullable<NumericVector> v0_ = R_NilValue) 
{
  ...
  for(int fold = 0; fold < folds; fold++) {
    ...
    for(int ll_iter = 0 ; ll_iter < n_ll; ll_iter++) {
      ...
      lr_(A_cv, X_cv, y_cv, u_cv, v_cv, beta_cv, u_distance_min, d_cv, ll, tol, max_iter, verbose, true, early_stopping, only_u, only_v);
  ...
  bool uv_is_init = false;
  if(u0_.isNotNull() & v0_.isNotNull()) {u_min = u0; v_min = v0; d_min = d0(0); uv_is_init = true; }
  iter_min = lr_(A, X, y, u_min, v_min, beta_min, u_distance_min, d_min, l_min, tol, max_iter, verbose, uv_is_init, early_stopping, only_u, only_v);
        ...
        bool early_stopping) 
{
  ...
  // aux
  bool cond = 1, early_nonstopping_cond = 1; 
  arma::colvec u_old, v_old, beta_old;
  std::deque<double> differences_u(200, 0.0), differences_v(200, 0.0);
  double d_old = 0;
  int iter = 0;
  ...
  while(cond & (iter < max_iter) & early_nonstopping_cond) {
    u_old = u; v_old = v; beta_old = beta; d_old = d;
    
    update_beta(X, u, v, y, beta);
    update_u_3(A, X, u, v, y, beta, d, l);
    update_v_3(A, X, u, v, y, beta, d, l);
    d = update_d(A, u, v);
    d /= pow(arma::norm(u, 2), 2);
    d /= pow(arma::norm(v, 2), 2);
    
    // scale u, v
    double u_scaling = sqrt(n) / arma::norm(u, 2);
    double v_scaling = sqrt(n) / arma::norm(v, 2);
    u *= u_scaling;
    v *= v_scaling;
    // scale betau, betav
    beta(beta.n_elem - 2) /= u_scaling;
    beta(beta.n_elem - 1) /= v_scaling;
    // scale d
    d /= (u_scaling * v_scaling);
    ...
    if(early_stopping) {
      ...
      double diff_u_norm = spec_norm(diff_u);
      double diff_v_norm = spec_norm(diff_v);
      
      differences_u.pop_front();
      differences_u.push_back(diff_u_norm);
      
      differences_v.pop_front();
      differences_v.push_back(diff_v_norm);
      
      if (iter >= 200) {
        for (int i = 0; i < 100; ++i) {
          if (diff_u_norm > differences_u[i] | diff_v_norm > differences_v[i]) {
            early_nonstopping_cond = false;
          }
        }
      }
        ...
        bool v_only) 
{
  ...
  int n_obs = X.n_rows, k = X.n_cols, n = A.n_rows;
  ...
  // std::cout << "\t\t U norm: " << arma::norm(u) << std::endl;
  // std::cout << "\t\t V norm: " << arma::norm(v) << std::endl;
  ...
  while( cond & (iter < max_iter) & early_nonstopping_cond)  {
    ...
    if(u_only) {
      ...
      beta(k+1) = 0;
      update_u_3(A, X, u, v, y, beta, d, l);
      update_v_3(A, X, u, v, d);
    }
    
    if(v_only) {
      update_beta(X, v, y, beta);
      beta.set_size(k+2);
      beta(k+1) = beta(k);
      beta(k) = 0;
      update_u_3(A, X, u, v, d);
      update_v_3(A, X, u, v, y, beta, d, l);
    }
    ...
    // scale u, v
    double u_scaling = sqrt(n); 
    double v_scaling = sqrt(n); 
    u_scaling /= arma::norm(u, 2);
    v_scaling /= arma::norm(v, 2);
    u *= u_scaling;
    v *= v_scaling;
    // scale betau, betav
    beta(beta.n_elem - 2) /= u_scaling;
        ...
        bool early_stopping) 
{
  ...
  while(cond & (iter < max_iter) & early_nonstopping_cond) {
    ...
    update_beta(X, u, v, y, beta);
    std::cout <<  iter << ": obj after beta: " << compute_objective(A, X, y, beta, d, u, v, l) << std::endl; 
    
    update_u_3(A, X, u, v, y, beta, d, l);
    std::cout <<  iter << ": obj after u: " << compute_objective(A, X, y, beta, d, u, v, l) << std::endl; 
    
    update_v_3(A, X, u, v, y, beta, d, l);
    std::cout <<  iter << ": obj after v: " << compute_objective(A, X, y, beta, d, u, v, l) << std::endl; 
    ...
    std::cout <<  iter << ": obj: " << compute_objective(A, X, y, beta, d, u, v, l) << std::endl; 
    
    std::cout <<  iter << ": d: " << d << std::endl;
    std::cout <<  iter << ": u norm: " << norm(u,2) << std::endl;
    std::cout <<  iter << ": v norm: " << norm(v,2) << std::endl;
    std::cout <<  iter << ": beta: " << beta.t() << std::endl;
    // scale u, v
    double u_scaling = sqrt(n) / arma::norm(u, 2);
    double v_scaling = sqrt(n) / arma::norm(v, 2);
    // std::cout <<  iter << ": u scale: " << norm(u*u_scaling,2) << std::endl;
    // std::cout <<  iter << ": v scale: " << norm(v*v_scaling,2) << std::endl;
    // std::cout <<  iter << ": betau scale: " << beta(beta.n_elem - 2) / u_scaling << std::endl;
    // std::cout <<  iter << ": betav scale: " << beta(beta.n_elem - 1) / v_scaling << std::endl;
    // std::cout <<  iter << ": d scale: " << d/(u_scaling * v_scaling) << std::endl;
    u *= u_scaling;
    v *= v_scaling;
    // scale betau, betav
    beta(beta.n_elem - 2) /= u_scaling;
    ...
    // scale d
    d /= (u_scaling * v_scaling);
    std::cout <<  iter << ": u scale: " << norm(u,2) << std::endl;
    std::cout <<  iter << ": v scale: " << norm(v,2) << std::endl;
    std::cout <<  iter << ": betau scale: " << beta(beta.n_elem - 2) << std::endl;
    std::cout <<  iter << ": betav scale: " << beta(beta.n_elem - 1) << std::endl;
    std::cout <<  iter << ": d scale: " << d << std::endl;
    std::cout <<  iter << ": obj scale: " << compute_objective(A, X, y, beta, d, u, v, l) << std::endl; 	
    ...
    if(verbose) {
      std::cout << "\t\t" << iter << ": " << beta.t() << std::endl;
      if(verbose == 3) {
        std::cout << "\t\t" << "u: " << spec_norm(u_old * u_old.t() - u * u.t()) << "; v: " << spec_norm(v_old * v_old.t() - v * v.t()) << "; beta: " << norm(beta - beta_old) << "; d: " << (d - 1) << std::endl;
      }
    }
    
    arma::mat diff_u = (u0 * u0.t() - u * u.t());
    u_distance(iter) = spec_norm(diff_u);
    
    // arma::mat diff_v = (v0 * v0.t() - v * v.t());
    // v_distance(iter) = spec_norm(diff_v);
    // v_distance(iter) = beta(beta.n_elem - 2) - 16;
    arma::mat diff_v = (u_old * u_old.t() - u * u.t());
    v_distance(iter) = spec_norm(diff_v);
    
    double objective = compute_objective(A, X, y, beta, d, u, v, l);
    obj(iter) = objective;
    ...
    if(early_stopping) {
      ...
      if (iter >= 200) {
        for (int i = 0; i < 100; ++i) {
          // std::cout << "\t\t Early stopping u " << diff_u_norm << std::endl;
          // std::cout << "\t\t Early stopping u_old " << differences_u[i] << std::endl;
          // std::cout << "\t\t Early stopping v " << diff_v_norm << std::endl;
          // std::cout << "\t\t Early stopping v_old " << differences_v[i] << std::endl;
          if (diff_u_norm > differences_u[i] | diff_v_norm > differences_v[i]) {
            early_nonstopping_cond = false;
            std::cout << "\t\t Early stopping at " << iter << std::endl;
          }
        }
      }
    }
                ...
                bool v_only) 
{
  double d;
  if(u_only) {
    d = as_scalar(u.t() * A * v_svd);
  } else if (v_only) {
    d = as_scalar(u_svd.t() * A * v);
  } else {
    d = as_scalar(u.t() * A * v);
  }
               ...
               double l) 
{
  ...
  arma::colvec rmat = beta(k)*(y - X*beta.head(k) - beta(k+1)*v.head(n_obs))/n_obs + l*d*A.head_rows(n_obs)*v/n/n;
  u.head(n_obs) = rmat / (pow(beta(k),2)/n_obs + l*pow(d,2)*pow(v_norm,2)/n/n);
               ...
               double l) 
{
  ...
  arma::colvec rmat = beta(k+1)*(y - X*beta.head(k) - beta(k)*u.head(n_obs))/n_obs + l*d*A.head_cols(n_obs).t()*u/n/n;
  v.head(n_obs) = rmat / (pow(beta(k+1),2)/n_obs + l*pow(d,2)*pow(u_norm,2)/n/n);
...
int update_u_3(const arma::mat& A,
               ...
               double d) 
{
  int n_obs = X.n_rows, n = A.n_rows, n_c = n - n_obs;
  double v_norm = arma::norm(v, 2);
  
  u.head(n_obs) = A.head_rows(n_obs)*v/d/pow(v_norm,2);
               ...
               double d) 
{
  ...
  v.head(n_obs) = A.head_cols(n_obs).t()*u/d/pow(u_norm,2);
...
) 
{
  ...
  svd_econ(U_diff, s_diff, V_diff, A);
...
// [[Rcpp::export]]
double compute_objective(const arma::mat& A, const arma::mat& X, const arma::colvec& y, 
                         const arma::colvec& beta, double d, 
                         const arma::colvec& u, const arma::colvec& v, double l) 
{
  int n = A.n_rows;
  // Concatenate X, u, and v horizontally
  arma::mat W = arma::join_horiz(X, u, v);
  
  // Calculate the objective function
  double term1 = arma::accu(arma::square(y - W * beta));
  double term2 = l * std::pow(arma::norm(A - d * u * v.t(), "fro"), 2)/n;
